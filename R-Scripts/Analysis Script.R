...
ts_plot(stream_tbl, by = "days", trim = 0L, tz = "UTC", ...)
...
# pull twitter data using rtweet
sample.set <- sample(stream_tbl$id_str, size = 200, replace = FALSE, prob = NULL)
sample.statuses <- lookup_statuses(sample.set)
sample.retweet.avg <- mean(sample.statuses$retweet_count)
sample.favorite.avg <- mean(sample.statuses$favorite_count)
sample.follower.avg <- mean(sample.statuses$followers_count)# Read in JSON Files
Stream <- stream_in(file(jsonfile))
Stream_flat <- jsonlite::flatten(Stream) #Flatten the JSON Filw into a table
stream_tbl <- as_tibble(Stream_flat) #create a dataframe from the flattened JSON Table
stream_tbl = stream_tbl[!grepl("RT", stream_tbl$text),]  #Removes all retweets from the dataframe
...
sample.retweet.avg <- mean(sample.statuses$retweet_count)
sample.favorite.avg <- mean(sample.statuses$favorite_count)
sample.follower.avg <- mean(sample.statuses$followers_count)
